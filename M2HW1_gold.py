...
def hanoi():
        ...
        p2 = []
        p3 = []
        print('Your goal is to move both discs from peg 1 to peg 3 while obeying the rules of the game:\n \
    •	You can only remove the top disc on a peg\n \
    •	Discs can only rest on empty pegs or on discs larger than they are.\n \
    You have three pegs (1, 2, 3). You have two discs (A, B). Disc A is smaller than Disc B.\n  (Remember, you can never have a larger disc on top of a smaller one.')
        while p3 != ['A','B']:
            print('1\t\t2\t\t3')
            print(p1,'\t',p2,'\t\t',p3)
            peg = input('Which peg do you want to move from? ').lower()
            post = input('Which peg do you want to move to? ').lower()
#            print(peg,post)
            if peg == '1' and post == '2':
                disc = p1.pop(0)
                print(disc)
                p2.append(disc)
            elif peg == '2' and post == '3':
                disc = p2.pop(0)
                print(disc)
                p3.insert(0,disc)
            elif peg == '1' and post == '3':
                disc = p1.pop(0)
                print(disc)
                p3.insert(0,disc)            
        print('You moved the discs')
        main()
